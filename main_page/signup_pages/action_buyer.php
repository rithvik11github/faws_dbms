...
if(!empty($name) || !empty($age) || !empty($address) || !empty($contactno) || !empty($aadharno) || !empty($password)|| !empty($username) ){
 $host = "localhost";
    ...
    if (mysqli_connect_error()) {
     ...
     $SELECT = "SELECT aadharno From buyer Where aadharno = ? Limit 1";
     $INSERT = "INSERT Into buyer(username, name, age, address, contactno, aadharno) values(?, ?, ?, ?, ?, ?)";
     $INSERT1 = "INSERT Into buyer_login(username, password) values (?, ?)";
     //Prepare statement
